...
export function RoadmapBoard({ topics, onEdit, onDelete }: { topics: LearningTopic[]; onEdit: (t: LearningTopic) => void; onDelete: (id: string) => void }) {
  if (topics.length === 0) {
    return <p className="text-sm text-muted-foreground">No topics yet — add your first one above, with whatever group makes sense to you.</p>;
  }

  const groups = uniqueGroups(topics);

  return (
    <div className="grid gap-4 md:grid-cols-3">
      {groups.map((group) => {
        const items = topics.filter((t) => (t.group.trim() || "Other") === group);
        return (
          <div key={group} className="rounded-2xl bg-accent/40 p-4">
            ...
            <ul className="space-y-3">
              {items.map((topic) => {
                const pct = topicProgressPct(topic);
                return (
                  <li key={topic.id}>
